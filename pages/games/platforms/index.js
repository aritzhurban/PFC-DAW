  ...
  function fetchItems() {
    ...
    fetch(initialURL)
      ...
      .then((data) => {
        ...
        setItems(mappedItems.filter(Boolean));
      })
      .catch((error) => {
    ...
    <div className={styles.content}>
      <header className={styles.content_header}>
        <div className={styles.content_header_title}>
          <h1 className={styles.heading}>Platforms</h1>
        </div>
      </header>
            ...
            >
              <div className={styles.card_info}>
                <div className={styles.card_info_header}>
                  <Link href={`/games/platforms/${item?.id}`}><span className={`${styles.card_link} ${styles.card_header_title}`}>{item?.name}</span></Link>
                </div>
